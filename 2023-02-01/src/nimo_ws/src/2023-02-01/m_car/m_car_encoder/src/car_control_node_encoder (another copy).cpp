...
void NavCarControlCallback(const geometry_msgs::Twist& msg)
{
  //ROS_INFO("I heard: [%s]", msg->data.c_str());
   ...
   steering_angle = (int)(-(msg.angular.z*180/3.14159)+STEER_NEUTRAL_ANGLE) ;
  
   
   if(steering_angle >=  30+STEER_NEUTRAL_ANGLE)  steering_angle = STEER_NEUTRAL_ANGLE+30;
   if(steering_angle <=  -30+STEER_NEUTRAL_ANGLE)  steering_angle = STEER_NEUTRAL_ANGLE-30;
   
   Base_Speed  = (int)(msg.linear.x*600);
   motor_speed = Base_Speed;
   if(motor_speed>=250)   motor_speed = 250;
   if(motor_speed<=-250)  motor_speed = -250;
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber sub4 = n.subscribe("/mobile_base/mobile_base_controller/cmd_vel", 10, &NavCarControlCallback);
