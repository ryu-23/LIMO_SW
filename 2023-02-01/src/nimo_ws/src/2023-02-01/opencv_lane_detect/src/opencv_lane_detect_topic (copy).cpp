...
void read_roi_data(void)
{
	...
	FILE *fp;
	int result = 0;
	fp = fopen("//home//amap//AA_data//vision_roi.txt","r");
	...
	result = fscanf(fp,"%d", &lane_distance);
...
void lane_detection(void)
{
   ...
   float c_x_sum=0;	     
    ...
    {
		  Init_lane_detect_ROI_l(); Init_lane_detect_ROI_r();
	      vision_right_lane_detection();
          vision_left_lane_detection();      
	}	  
	...
	if( (left_line_detect_flag != 0) && (right_line_detect_flag != 0))
    {
	  ...
	  lane_center_x = (l_line_center + r_line_center)/2 -img_width/2;
	...
	else if( (left_line_detect_flag == 0) && (right_line_detect_flag != 0))
    {
	  ...
	  lane_center_x = (-lane_distance/2 + r_line_center) -img_width/2;
	...
	printf("L:%4.1f R:%4.1f C: %4.3f %d %d\n", l_line_center, r_line_center, lane_center_x, left_line_detect_flag,right_line_detect_flag);
	error_lane = lane_center_x;
	
	steer_angle_new = (int)( error_lane *Kp + Kd*error_lane_delta/1.42857);  //스티어링 조정값 계수 수정 필요
...
int main(int argc, char **argv)
{
   ...
   */
   ...
   ros::Publisher car_control_pub_cmd = nh.advertise<std_msgs::Int16>("/steering/mode_0", 10);
   //ros::Publisher car_speed_pub_cmd = nh.advertise<geometry_msgs::Twist>("/cmd_vel/mode_3", 10);
  ...
  ////////////////  image display window ///////////////////////////
   ...
   //namedWindow("Right Edge Image", WINDOW_NORMAL);
   //resizeWindow("Camera Image", IMG_Width/2,IMG_Height/2);
   //moveWindow("Right Edge Image", 450, 10); 
   
   
   /*
   namedWindow("Left Edge Image", WINDOW_NORMAL);
   //resizeWindow("Camera Image", IMG_Width/2,IMG_Height/2);
   moveWindow("Left Edge Image", 10, 10);
   
   
   namedWindow("Left Lane Image", WINDOW_NORMAL);
   //resizeWindow("Camera Image", IMG_Width/2,IMG_Height/2);
   moveWindow("Left Lane Image", 10, 400);
   
   namedWindow("Right Lane Image", WINDOW_NORMAL);
   //resizeWindow("Camera Image", IMG_Width/2,IMG_Height/2);
   moveWindow("Right Lane Image", 450, 400); 
   */
