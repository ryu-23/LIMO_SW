...
#define MAX_L_STEER -80
#define MAX_R_STEER 80
#define STEER_NEUTRAL_ANGLE 90

#define RAD2DEG(x) ((x)*180./M_PI)
#define DEG2RAD(x) ((x)/180.*M_PI)
// unit : m
#define Sonar_Detect_Range 0.01
...
void NavCarControlCallback(const geometry_msgs::Twist& msg)
{
  //ROS_INFO("I heard: [%s]", msg->data.c_str());
  
   steering_angle = (int)(-(msg.angular.z*180/3.14159)*2.5+STEER_NEUTRAL_ANGLE) ;
  
   
   if(steering_angle >=  MAX_R_STEER+STEER_NEUTRAL_ANGLE)  steering_angle = STEER_NEUTRAL_ANGLE+MAX_R_STEER;
   if(steering_angle <=  MAX_L_STEER+STEER_NEUTRAL_ANGLE)  steering_angle = STEER_NEUTRAL_ANGLE+MAX_L_STEER;
   
   Base_Speed  = (int)(msg.linear.x*1800);
   motor_speed = Base_Speed;
   if(motor_speed>=250)   motor_speed = 250;
   if(motor_speed<=-250)  motor_speed = -250;
...
void LidarCarSteerCallback(const std_msgs::Int16& angle)
{
  steering_angle = (int)(angle.data+STEER_NEUTRAL_ANGLE) ;
...
void LidarCarSpeedCallback(const geometry_msgs::Twist& speed)
{
  Base_Speed = (int)speed.linear.x;
  motor_speed = Base_Speed;
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber sub5 = n.subscribe("/mobile_base/mobile_base_controller/cmd_vel", 10, &NavCarControlCallback);
  
  //lidar obstacle avoidance
  ros::Subscriber sub6 = n.subscribe("/Car_Control_cmd/lidar_steerAngle_Int16", 10, &LidarCarSteerCallback);
  ros::Subscriber sub7 = n.subscribe("/Car_Control_cmd/lidar_Speed_Int16", 10, &LidarCarSpeedCallback);
  //
  ...
  while (ros::ok())
  {
   ...
   //odom.twist.twist.angular.z = myOdomCaculateData.velocity_angular;
     ...
     ROS_INFO("Encoder : %ld Distance : %6.3lf", encoder, (float)encoder/myOdomCaculateData.distance_ratio);//myOdomCaculateData.distance_ratio);///
	if((count %= 10) == 0)
    {
