...
void lane_detection(void)
{
   ...
   float c_x_sum=0;	     
	...
	else if( (left_line_detect_flag != 0) && (right_line_detect_flag == 0))
    {
	  ...
	  lane_center_x = ((l_line_center + lane_distance/2) -img_width/2)+50;
	  //steer_angle_new = 0.0994*lane_center_x-18.07;
	  // angle = a*lane_center_x + b 
	  steer_angle_new = 0.82*(0.0965*lane_center_x+18.902);
		  
	}
	else if( (left_line_detect_flag == 0) && (right_line_detect_flag != 0))
    {
		  
	  lane_center_x = ((-lane_distance/2 + r_line_center) -img_width/2)-50 ;
	  steer_angle_new = 0.82*(0.0994*lane_center_x-18.07);
