...
int mission_flag = 0, odom_flag =0, odom_init_flag =0;
int Init_yaw_flag = 0;
...
geometry_msgs::Twist mux_mission_msg;
...
int main(int argc, char **argv)
{
	...
	ros::Publisher  mux_mission_pub =  n.advertise<geometry_msgs::Twist>(mux_mission_output_topic, 10);
	...
	while(ros::ok())
  {
	  ...
	  switch(mission) {
		  case 0:{
			...
			if(lidar_detected == 1 && mission_flag == 0){
				printf("Let'GO");
				odom_init_flag = 1;
		...
		break;
		  ...
		  {   printf("333Let'GO3333");
			  if(odom_flag == 0)
			  {
				  if((position_x-init_position_x) > 0.40){ // 0.6 , 0.35
					cmd_vel_msg.linear.x = 0.0;
					cmd_vel_msg.angular.z = 0.0;
					odom_flag = 1;
				 }
				   ...
				   if((position_x-init_position_x) > 0.9){ // 0.6 , 0.35
					cmd_vel_msg.linear.x = 0.0;
					cmd_vel_msg.angular.z = 0.0;
					odom_flag = 2;
				 }
	...
	mux_mssion_pub.publish(mux_mission_msg);
