...
//////////////////////////
int crosswalk_detected = 0;
int crosswalk_distance = 0;
float steer_angle_new2 =0;
//////////////////////////
float steer_angle_old; 
...
//////////////////////////////////////// PID control ////////////////////////////////////////////////
float  Kp = 0.01;  float Ki = 0; float  Kd = 0.04;
...
void lane_detection(void)
{
   ...
   float c_x_sum=0;	     
	...
	if( (left_line_detect_flag != 0) && (right_line_detect_flag != 0))
    {
	  ...
	  steer_angle_new2 = error_lane *Kp + Kd*error_lane_delta/1.42857;
	  steer_angle_new = RAD2DEG(steer_angle_new2);
	  speed = 0.3;
	...
	else if( (left_line_detect_flag != 0) && (right_line_detect_flag == 0))
    {
	  ...
	  lane_center_x = ((l_line_center + lane_distance/2) -img_width/2);
	  steer_angle_new2 = error_lane *Kp + Kd*error_lane_delta/1.42857;
	  steer_angle_new = RAD2DEG(steer_angle_new2);
	  speed = 0.25;
	...
	else if( (left_line_detect_flag == 0) && (right_line_detect_flag != 0))
    {
	  ...
	  lane_center_x = ((-(lane_distance+80)/2 + r_line_center) -img_width/2);
	  steer_angle_new2 = (error_lane *Kp + Kd*error_lane_delta/1.42857);
	  steer_angle_new = RAD2DEG(steer_angle_new2);
	  
	  speed = 0.25;
	  //steer_angle_new = 0.62*(0.0994*lane_center_x-18.07);
...
void crosswalk_y_callback(const std_msgs::Int32 & msg)
{
	if (msg.data == -1){
...
double control_yaw(void)
{
  ...
  //printf("error %6.3lf  %6.3lf \n", error1, error2);
    ...
    error_y = error1;
    //if(fabs(error1) > fabs(error2))  error_y = error2;
    //else error_y = error2;
...
int main(int argc, char **argv)
{
   ...
   */
   ...
   nh.param("subscribers/camera_reading/topic", cameraTopicName, std::string("/camera/rgb/image_raw"));
   nh.param("subscribers/camera_reading/queue_size", cameraQueueSize, 1);
   ///////////////imu///////////////
   ros::Subscriber subIMU =  nh.subscribe(imu_topic, 20, imuCallback);
   /////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////////////
  //rospy.Subscriber("/limo/crosswalk_y", Int32, self.crosswalk_y_callback)
   //ros::Subscriber crosswalk_sub = nh.subscribe("/limo/crosswalk_y", 10, crosswalk_y_callback);
   //car_control_pub_cmd = nh.advertise<std_msgs::Float32>("/yaw_target_topic", 10);
   car_speed_pub_cmd = nh.advertise<geometry_msgs::Twist>("/cmd_vel_lane", 10);
   ...
   { 
     ...
     */
     ...
     Pid_out = control_yaw();
	  ...
	  cmd_speed_msg.angular.z = Pid_out;
	  cmd_speed_msg.linear.x = speed;
